...
export default function SignUpPage() {
  ...
  useEffect(() => {
    const checkRedirectResult = async () => {
      ...
      try {
        ...
        if (result) {
          console.log("DEBUG: Redirect result found for user:", result.user.email);
          setIsLoading(true);
          await syncUser(result.user);
        }
      } catch (err: any) {
        console.error("Redirect result error:", err);
        setError(`Authentication Error (${err.code}): Please ensure your domain is authorized in Firebase.`);
      }
    };
    checkRedirectResult();
  }, []);
  ...
  const handleGoogleLogin = async () => {
    setError("")
    setIsLoading(true)
    console.log("DEBUG: Starting Google Login Redirect...");
    console.log("DEBUG: current origin:", window.location.origin);
    try {
      const provider = new GoogleAuthProvider();
      await signInWithRedirect(auth, provider);
    } catch (err: any) {
      console.error("Google login redirect prompt error:", err);
      setError(`Auth Error (${err.code || 'unknown'}): Domain may not be authorized.`);
      setIsLoading(false)
    }
  }
