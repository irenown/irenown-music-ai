...
export function ResultsView({ projectData, onNewProject }: ResultsViewProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
            ...
            {Array.from({ length: 100 }).map((_, i) => (
              <div
                ...
                className={`w-1 rounded-full transition-all ${
                  i < (currentTime / projectData.duration) * 100
      ...
      <div className="flex flex-wrap gap-3">
        <Button className="flex-1 sm:flex-none bg-primary text-primary-foreground hover:bg-primary/90 glow-primary gap-2">
          <Download className="h-4 w-4" />
          Download {projectData.qualityTier === "premium" ? "All Formats" : "MP3"}
          <ChevronDown className="h-4 w-4" />
        </Button>
        <Button variant="outline" className="flex-1 sm:flex-none border-border bg-card/50 gap-2">
        ...
        <Button variant="outline" onClick={onNewProject} className="flex-1 sm:flex-none border-border bg-card/50 gap-2">
